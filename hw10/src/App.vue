...
export default {
    ...
    beforeCreate() {
      ...
      this.$root.$on("onWriteComment", (text, postId) => {
        if (this.userId) {
          if (!text.trim()) {
            this.$root.$emit("onWriteCommentValidationError", "Enter text");
          ...
          this.$root.$emit("onWriteCommentValidationError", "Please login");
        }
      });
      ...
      this.$root.$on("onRegister", (login, name) => {
        if (!login || login.length < 3) {
          ...
          this.$root.$emit("onRegisterValidationError", "Login can only consist of lowercase Latin letters");
        } else if (!name.trim()) {
          this.$root.$emit("onRegisterValidationError", "Name is too short");
        } else if (name.length > 32) {
          this.$root.$emit("onRegisterValidationError", "Name is too long");
        } else if (Object.values(this.users).filter(usr => usr.login === login).length > 0) {
