...
public class FrontServlet extends HttpServlet {
    ...
    private boolean checkMethod(Method method, Route route, Map<Class<?>, Object> map) {
        return method.getName().equals(route.getAction()) && Arrays.stream(method.getParameterTypes()).allMatch(map::containsKey);
    }
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Map<Class<?>, Object> mapOfArgs = Map.of(
                Map.class, view,
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method currMethod : clazz.getDeclaredMethods()) {
                if (checkMethod(currMethod, route, mapOfArgs)) {
                    method = currMethod;
                    break;
                }
            }
        ...
        for (Class<?> methodType : method.getParameterTypes()) {
            if (mapOfArgs.get(methodType) != null) {
                arguments[index] = mapOfArgs.get(methodType);
            }
            index++;
        }
        try {
        ...
        if (language != null && !language.equals("en")) {
            langTemp = "_" + request.getSession().getAttribute("lang").toString();
        }

//        Template template = pageClass.getSimpleName().equals("IndexPage") ? newTemplate(pageClass.getSimpleName() + langTemp + ".ftlh") : newTemplate(pageClass.getSimpleName() + ".ftlh");

        Template template;
        try {
            sourceConfiguration.getTemplate(pageClass.getSimpleName() + langTemp + ".ftlh");
            template = newTemplate(pageClass.getSimpleName() + langTemp + ".ftlh");
        } catch (TemplateNotFoundException ignored) {
            template = newTemplate(pageClass.getSimpleName() + ".ftlh");
        } catch (IOException e) {
            throw new ServletException("Can't load template [templateName=" + pageClass.getSimpleName() + ".ftlh" + "]", e);
        }
